...
export class FungsiPage {
  ...
  getFungsi() {
    ...
    this.fungsi.getFungsi({}).subscribe(
      (res) => {
        console.log(res);
  ...
  createUpdateFungsi(modal) {
    ...
    if (value._id) {
      this.fungsi.updateFungsi(value).subscribe(
        (res) => {
          console.log(res);
      ...
      this.fungsi.createFungsi(value).subscribe(
        (res) => {
          console.log(res);
          this.loadingCreateUpdateFungsi = false;
          this.getFungsi();
  ...
  async actionUpdateItem(modalItemAction, modalCreateUpdateFungsi, id) {
    ...
    this.formCreateUpdateFungsi.patchValue({
      ...
      namaOrganisasi:
        fungsi.organisasi?.nama +
        (fungsi.organisasi.tipe == 'wilayah'
          ? ' - ' + fungsi.organisasi.zona.nama
          : ''),
  ...
  async actionDeleteItem(modal, id) {
    ...
    this.fungsi.deleteFungsi(id).subscribe(
      (res) => {
        console.log(res);
        loading.dismiss();
        modal.dismiss();
  ...
  openModalSelectOrganisasi(modal) {
    ...
    this.fungsi.getOrganisasi({}).subscribe(
      (res) => {
        console.log(res);
        this.loadingDataSelectOrganisasi = false;
        this.dataSelectOrganisasi = res.organisasi;
        this.selectedOrganisasi = res.organisasi.find(
          (v) => v._id == this.formCreateUpdateFungsi.get('organisasi').value
        );
      },
      (err) => {
        console.log(err);
  ...
  selectOrganisasi(modal) {
    ...
    this.formCreateUpdateFungsi.patchValue({
      ...
      namaOrganisasi:
        this.selectedOrganisasi?.nama +
        (this.selectedOrganisasi.tipe == 'wilayah'
          ? ' - ' + (this.selectedOrganisasi as Wilayah).zona.nama
          : ''),
    });
    modal.dismiss();
  }
